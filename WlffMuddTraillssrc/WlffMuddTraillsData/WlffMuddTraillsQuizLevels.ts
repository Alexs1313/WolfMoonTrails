export type WlffMuddTraillsQuizOption = {id: string; text: string; correct: boolean};
export type WlffMuddTraillsQuizQuestion = {
  id: string;
  text: string;
  options: WlffMuddTraillsQuizOption[];
  explanation: string;
};
export type WlffMuddTraillsQuizLevel = {
  id: string;
  title: string;
  order: number;
  questions: WlffMuddTraillsQuizQuestion[];
};
...
export const WlffMuddTraillsQuizLevels: WlffMuddTraillsQuizLevel[] = [
  {
    id: 'trail-awareness',
...
export function WlffMuddTraillsGetQuizLevel(id: string): WlffMuddTraillsQuizLevel | undefined {
  return WlffMuddTraillsQuizLevels.find(level => level.id === id);
}

export function WlffMuddTraillsGetNextLevelId(id: string): string | undefined {
  const index = WlffMuddTraillsQuizLevels.findIndex(level => level.id === id);
  if (index === -1 || index >= WlffMuddTraillsQuizLevels.length - 1) {
    return undefined;
  }
  return WlffMuddTraillsQuizLevels[index + 1].id;
}

export function WlffMuddTraillsGetStarCount(correct: number, total: number): number {
  if (total <= 0) {
    return 0;
  }
  if (correct >= total) {
